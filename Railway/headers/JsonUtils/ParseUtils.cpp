...
namespace {
	std::vector<Train> ParseTrains(const rapidjson::GenericArray<false, rapidjson::Value>& array);
	std::unique_ptr<Town> ParseTown(const rapidjson::Value& town_item);
	std::unique_ptr<Town> ParseTown(const rapidjson::Value& town_item);
	std::unique_ptr<Market> ParseMarket(const rapidjson::Value& market_item);
	std::unique_ptr<Storage> ParseStorage(const rapidjson::Value& storage_item);
}
...
Player ParsePlayer(std::string input) {
	...
	return move(Player{ home, move(idx), in_game, move(name), rating, move(*town_ptr), move(trains) });
}
